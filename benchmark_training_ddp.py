#!/usr/bin/env python3
"""
Pluto model multi-GPU DDP training speed benchmark with simulated data.
...
Note: --batch-size is per-GPU. Total batch = batch_size * num_gpus.
"""
...
def generate_fake_batch(
    ...
    bs, A, M, P = batch_size, num_agents, num_polygons, num_points_per_polygon
    ...
    data = {
        ...
        "cost_maps": torch.rand(bs, cost_map_size, cost_map_size, 1, device=device),
    }
    return data
...
def compute_loss(res, data, collision_loss_fn, num_modes, radius, history_steps):
    ...
    target_label[torch.arange(bs, device=trajectory.device), target_r_index, target_m_index] = 1

    best_traj = trajectory[torch.arange(bs, device=trajectory.device), target_r_index, target_m_index]
...
def benchmark(args):
    ...
    log("=" * 60)
    log("PLUTO DDP TRAINING BENCHMARK")
    log("=" * 60)
    log(f"Mode: {'DDP ({} GPUs)'.format(world_size) if ddp_enabled else 'Single GPU'}")
    log(f"GPU: {torch.cuda.get_device_name(device)}")
    log(f"GPU Memory: {torch.cuda.get_device_properties(device).total_memory / 1e9:.1f} GB per card")
    ...
    log(f"History steps: {args.history_steps}, Future steps: {args.future_steps}")
    log(f"Agents: {args.num_agents}, Polygons: {args.num_polygons}")
    log("-" * 60)
    ...
    for _ in range(args.warmup_steps):
        ...
        res = model(data)
        loss, _ = compute_loss(
            res, data, collision_loss_fn,
            raw_model.num_modes, raw_model.radius, args.history_steps,
        )
        loss.backward()
        optimizer.step()
        optimizer.zero_grad()
    ...
    # Benchmark
    log(f"Benchmarking {args.num_steps} steps...")
    step_times = []
    fwd_times = []
    bwd_times = []
    optim_times = []
    ...
    for step in range(args.num_steps):
        ...
        # Backward (DDP gradient sync happens inside backward)
        loss.backward()
        torch.cuda.synchronize(device)
        ...
        step_times.append(t4 - t0)

        if is_main_process() and ((step + 1) % 10 == 0 or step == 0):
            print(
                f"  Step {step+1:4d} | loss={loss.item():.4f} | "
                f"total={step_times[-1]*1000:.1f}ms "
                f"(data={data_times[-1]*1000:.1f} fwd={fwd_times[-1]*1000:.1f} "
                f"bwd={bwd_times[-1]*1000:.1f} optim={optim_times[-1]*1000:.1f}ms)",
                flush=True,
            )
    ...
    # Collect timing stats from all ranks
    def stats(times):
        t = torch.tensor(times)
        return t.mean().item(), t.std().item(), t.median().item(), t.min().item(), t.max().item()

    # Gather step_times from all ranks to find the slowest
    if ddp_enabled:
        mean_step = torch.tensor([sum(step_times) / len(step_times)], device=device)
        all_means = [torch.zeros(1, device=device) for _ in range(world_size)]
        dist.all_gather(all_means, mean_step)
    else:
        all_means = [torch.tensor([sum(step_times) / len(step_times)])]
    ...
    if is_main_process():
        ...
        # Per-step stats (from rank 0, representative since workload is identical)
        print("  [Rank 0 Timing]")
        for name, times in [
            ("Total step", step_times),
            ("Data gen", data_times),
            ("Forward+Loss", fwd_times),
            ("Backward", bwd_times),
            ("Optimizer", optim_times),
        ]:
            mean, std, med, mn, mx = stats(times)
            print(
                f"    {name:18s}: mean={mean*1000:7.1f}ms | median={med*1000:7.1f}ms | "
                f"std={std*1000:5.1f}ms | min={mn*1000:7.1f}ms | max={mx*1000:7.1f}ms"
            )

        if ddp_enabled:
            print()
            print("  [Per-Rank Mean Step Time]")
            for i, m in enumerate(all_means):
                print(f"    Rank {i}: {m.item()*1000:.1f}ms")
            slowest = max(m.item() for m in all_means)
            fastest = min(m.item() for m in all_means)
            print(f"    Slowest: {slowest*1000:.1f}ms  |  Fastest: {fastest*1000:.1f}ms  |  Imbalance: {(slowest/fastest - 1)*100:.1f}%")

        # Effective throughput: global batch / slowest rank step time
        effective_step_time = max(m.item() for m in all_means)
        ...
        if ddp_enabled:
            # Scaling efficiency
            single_gpu_throughput = per_gpu_throughput  # ideal case
            ideal_global = single_gpu_throughput * world_size
            scaling_efficiency = global_throughput / ideal_global * 100 if ideal_global > 0 else 0
            print(f"    Scaling efficiency: {scaling_efficiency:.1f}% (ideal=100%)")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pluto DDP training speed benchmark")
